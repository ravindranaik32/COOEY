...
import com.biz.cooey.MultiSpinner;
import com.biz.cooey.MultiSpinner.MultiSpinnerListener;
...
public class AddDeviceActivity extends Activity {
			...
			case R.id.scanBtn: 
			{
				...
				if (mLsBleManager != null && mLsBleManager.isOpenBluetooth())
				{
					if (mLsBleManager.isSupportLowEnergy()) 
					{
						...
						{
							showPromptDialog("Warning", "Other tasks are running, please try again later",-1);
						} else 
						{
							showScanDialog();
						}
						...
						showPromptDialog("Prompt", "Not support Bluetooth Low Energy",-1);
					}

				} else {
					showPromptDialog("Prompt", "Please turn on the Bluetooth function",-1);
				}
				}break;
			case R.id.stopBtn: 
			{
	...
	private void setScanConitions() {
		...
		Spinner broSpinner = (Spinner)promptsView.findViewById(R.id.broadcastSpinner);
		// Prepar adapter
		...
		ArrayAdapter<String> adapter = new ArrayAdapter<String>(AddDeviceActivity.this,
				android.R.layout.simple_spinner_item, broStrings);
		adapter.setDropDownViewResource(android.R.layout.select_dialog_singlechoice);
		broSpinner.setAdapter(adapter);
		
		broSpinner.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
		...
		MultiSpinner multiSpinner = (MultiSpinner) promptsView.findViewById(R.id.deviceTypeSpinner);
		multiSpinner.setItems(test, "Please select",new MultiSpinnerListener() {
	...
	private void initListView() 
	{
		...
		scanResultsListView.setOnItemClickListener(new AdapterView.OnItemClickListener() 
		{
					...
					int position, long id) 
			{
				...
				BleDevice de = (BleDevice) parent.getAdapter().getItem(position);
				startPairDevice(de);
	...
	private void showProgressDialog(String title,String message)
	{
		ringProgressDialog = ProgressDialog.show(
				AddDeviceActivity.this, title, message, true);
		...
		if(userList!=null && userList.size()>0)
		{
			...
			for(DeviceUserInfo  user:userList)
			{
				if(user!=null)
				{
					...
					else items[index]=user.getUserNumber()+"   "+user.getUserName();
				}	
			}
			}
	...
	private void writeUserAccountToDevice(int userNumber,String name) 
	{
        ...
        showProgressDialog("Binding Device User","Binding,please wait...........");
	...
	private void updateListViewBackground(String name) 
	{
		...
		if (!tempList.isEmpty()) 
		{
			...
			{
				if (dev != null && dev.getName()!=null && dev.getName().equals(name)) 
				{
	...
	private BleDevice getBleDevice(String name, String deviceType,String broCastId,
			int pairStatus,String protocolType,String modelNumber) 
	{
	...
	private boolean deviceExists(String name) 
	{
		...
		if(tempList!=null && tempList.size()>0)
		{
			...
			{
				if (tempList.get(i)!=null && tempList.get(i).getName()!=null && tempList.get(i).getName().equals(name)) 
				{
					return true;
				}
			}
	...
	private void showScanDialog() {
		scanProgressDialog = ProgressDialog.show(AddDeviceActivity.this,
				"BroadcastType:"+getBroadcastType(broadcastType), "Scanning:"+scanDeviceBuffer+"............", true);
				...
				if (!scanResult) {
					AlertDialog.Builder builder = new AlertDialog.Builder(
							...
							.setPositiveButton("OK",
									...
									}).setMessage("No results!Please change scanning conditions and retry");
	...
	private String getBroadcastType(BroadcastType broType) {
		if(broType!=null)
		{
			...
			else return broType.toString();
		}
		else return "All Broadcast";
	...
	private void checkUserNumber(int userNumber) {
		...
		{
			userListDialog.dismiss();
			showPromptDialog("Confirm number",userNumber);
		}
		else 
		{
			int number=-1;
			if(!MainActivity.deviceUserNumberMap.isEmpty()&& MainActivity.deviceUserNumberMap.get(deviceId)!=null)
			{
